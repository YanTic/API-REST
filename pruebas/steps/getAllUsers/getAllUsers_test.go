...
func laAPIRespondeConLaListaDeUsuarios() error {
	...
	// SERA QUE CON ESTO YA????
	documentLoader := gojsonschema.NewBytesLoader(apiResponse.Body)
	res := gojsonschema.NewBytesLoader(schemaBytes)
	...
	result, err := gojsonschema.Validate(res, documentLoader)
	if err != nil {
